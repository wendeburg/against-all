...
AA_Engine escuchará por peticiones con el formato `{"npcid": npc-uuid}` donde npc-uuid es el identificador del npc. AA_Engine responderá con una oferta para request `{"npcid": npc-uuid, "token": token, "partida": idpartida}`. En respuesta el npc responderá con `{"npcid": npc-uuid, "accepted": token, "partida": idpartida}` donde token es la token que acepta.
